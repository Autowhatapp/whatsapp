...
const generateFlowJSON = (schema: Schema) => {
  ...
  router.post('/createflow', upload.none(), async (req: Request, res: Response) => {
...
router.get('/getflowid/:flowId', async (req: Request, res: Response) => {
  ...
  try {
    const response = await axiosInstance.get(`${flowId}`, {
      params: {
        fields: 'id,name,categories,preview,status,validation_errors,json_version,data_api_version,data_channel_uri,whatsapp_business_account,application'
      }
    });
    res.json(response.data);
  } catch (error) {
...
router.post('/updateflow/:flowId', upload.single('file'), async (req: FlowRequest, res: Response) => {
  const { flowId } = req.params;
  const file = req.file;
