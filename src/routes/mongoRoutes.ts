...
router.get('/users/:phone', async (req, res) => {
  try {
    ...
    const user = await collection.findOne({ phone: Number(req.params.phone) });
...
router.get('/user/:userId/workspacesowner', async (req, res) => {
  try {
    ...
    res.status(200).json({ error: 'Error fetching workspaces for user' });
  }
});
...
router.get('/bots/:id', async (req, res) => {
  try {
    ...
    if (bot) {
      ...
      res.status(200).json({ error: 'Bot not found' });
    }
  } catch (error) {
    console.error('Error fetching bot:', error);
    res.status(200).json({ error: 'Error fetching bot' });
  }
});
