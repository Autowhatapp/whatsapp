...
router.post('/orchestrate-flow', async (req: OrchestrationRequest, res: Response) => {
  const { schema, flowName, headerText, bodyText, footerText, customerPhoneNumber } = req.body;
console.log(schema, flowName, headerText, bodyText, footerText, customerPhoneNumber,"schema, flowName, headerText, bodyText, footerText, customerPhoneNumber")
  try {
    ...
    console.log(createFlowResponse.data.id,"createFlowResponse")
...
const customValue = "<CUSTOM_VALUE>"
    ...
    res.status(500).json({ error: 'An error occurred during flow orchestration' });
  }
});
